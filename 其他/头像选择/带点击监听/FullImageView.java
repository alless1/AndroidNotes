package com.sankeyun.bb.widget;
...
public class FullImageView extends ImageView implements ScaleGestureDetector.OnScaleGestureListener,
...
/*        getViewTreeObserver().addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
    ...
    public void initData(Drawable drawable){
        ...
        Log.e(TAG, "onGlobalLayout: mDrawableWidth = "+mDrawableWidth+" mDrawableHeight="+mDrawableHeight );
        initImageViewSize();
        moveToCenter();
    }
    ...
    public boolean onTouch(View v, MotionEvent event) {
        switch (event.getAction() & MotionEvent.ACTION_MASK) {
            case MotionEvent.ACTION_DOWN:
                ...
                downX = event.getX();
                downY = event.getY();
               // Log.e(TAG, "onTouch: downX = " + downX + "downY=" + downY);
            ...
            case MotionEvent.ACTION_UP:
               // Log.e(TAG, "onTouch:ACTION_UP upX =" + upX + "upY=" + upY);
                isDown = false;
                upX = event.getX();
                upY = event.getY();
    ...
    private void handleLongClick() {
        mHandler.postDelayed(new Runnable() {
            ...
            public void run() {
                if (!isDown)
                    return;
                if (nowMovingX != 0 && Math.abs(downX - nowMovingX) > 50)
                    return;
                if (nowMovingY != 0 && Math.abs(downY - nowMovingY) > 50)
                    return;
                //长按事件触发
               // Log.e(TAG, "run: 长按事件");
                if(mListener!=null)
    ...
    private void handleOnClick() {
        long interval = upTime - downTime;
        if (interval > 200)//触发点击事件200ms以内
            return;
        if (upX != 0 && Math.abs(downX - upX) > 50)
            return;
        if (upY != 0 && Math.abs(downY - upY) > 50)
            return;
      //  Log.e(TAG, "handleOnClick: 点击事件");
        if(mListener!=null)
            mListener.onClick();
