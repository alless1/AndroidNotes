...
//            Log.d(TAG, "initData: mHost ="+mHost +" mPort="+mPort);
    ...
    private Socket getSocket(String ip, int port) {
        ...
        try {
            ...
            socket.connect(new InetSocketAddress(ip, port), CONNECTION_TIMEOUT_SECOND);
            socket.setSoTimeout(READ_TIMEOUT_SECOND);
    ...
    private boolean chooseAuthentication(InputStream is, OutputStream os, String user, String password) throws IOException {
        ...
        if (METHOD == 0x02) {
            ...
            onError(ErrorCode.CRN_HANDSHAKE_FAIL, "socks5 handshake fail METHOD =" + METHOD);
            return false;
        }
    }
...
/*            if (isTerminated) {
                ...
                if (mListener != null) {
                  *//*  long endTime = System.currentTimeMillis();
                    ...
                    Log.e(TAG, "uploadFile 1 :onCompletion " );
                    onCompletion();
                    return;
                }
            }*/
           ...
           /* DataInputStream dataInputStream = new DataInputStream(mIs);
            ...
            byte[] buffer2 = new byte[LOCAL_WRITE_READ_BUFFER_SIZE];
            boolean eohFound = false;
            int bodyLength = 0;
            StringBuilder builder = new StringBuilder();
            while ((count = dataInputStream.read(buffer2)) != -1) {
                offset = 0;
                if (!eohFound) {
                    String stringHeader = new String(buffer2, 0, count);
                    int indexOfEOH = stringHeader.indexOf("\r\n\r\n");
                    if (indexOfEOH != -1) {
                        //处理包头
                        String content = "CONTENT-LENGTH: ";
                        int executeTask = stringHeader.toUpperCase().indexOf(content);
                        String contentLength = stringHeader.substring(executeTask + content.length());
